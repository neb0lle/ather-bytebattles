...
void asdk_app_init() {
    asdk_clock_config_t clk_cfg = {.clk_source = ASDK_CLOCK_SRC_INT,
                                   .pll =
                                       {
...
static void task_5ms(void) {
    ...
    if (get_current_filter() == BLUE_FILTER) {
        ...
        // colour_map.red = MAP(colour_c.red, RED_NO_LIGHT_VALUE,
        // RED_WHITE_LIGHT_VALUE); colour_map.green = MAP(colour_c.green,
        // GREEN_NO_LIGHT_VALUE, GREEN_WHITE_LIGHT_VALUE); colour_map.blue =
        // MAP(colour_c.blue, BLUE_NO_LIGHT_VALUE, BLUE_WHITE_LIGHT_VALUE);
...
static void task_1000ms(void) { app_ldr_iteration(); }
